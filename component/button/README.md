...
좋아요와 싫어요는 기존 메시지 버튼 클래스와 SVG 아이콘을 그대로 사용하고, `aria-pressed` 상태를 각각 독립적으로 토글합니다.
...
답변서 화면의 접힌 패널에서 사용하는 파일 추가 버튼은 기존 `.collapsed-add-btn` 구조를 그대로 사용합니다. 표시 조건은 상위 패널의 `.panel-folder.panel-collapsed` 상태입니다.

```html
<div class="panel-folder panel-collapsed">
    <button type="button" class="collapsed-add-btn" aria-label="파일 추가" title="파일 추가">
        <span class="collapsed-add-file-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24">
                <path d="M6 2h8l4 4v16H6z"/>
                <path d="M14 2v5h5"/>
            </svg>
            <span class="collapsed-add-plus">+</span>
        </span>
    </button>
</div>
```
...
| 접힌 패널 파일 추가 | `.collapsed-add-btn` 30×32px | 문서 20×23px + 배지 12×12px |
