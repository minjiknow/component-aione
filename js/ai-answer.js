(function () {
        ...
        const pages = [
            {
                ...
                sections: [
                    {
                        title: "1. 질의 배경",
                        desc: "재경위 소속 의원들은 지방교부세가 이미 지급되고 있는 상황에서 추가로 지방채 인수 추경을 편성하는 배경과 필요성을 집중적으로 질의하였습니다. 특히 일반재원과 목적성 재원의 차이, 지방채 인수의 법적 성격, 재정건전성에 미치는 영향 등을 함께 설명할 필요가 있다는 점이 검토의 출발점이 되었습니다.",
                        items: [
                            "지방교부세는 일반재원 성격을 가지나, 지방채 인수는 특정 자본적 지출 사업에 대한 목적성 재원 지원이라는 점을 우선 설명할 필요가 있음",
                            "의원 질의는 단순한 예산 증액 사유보다도 지방재정 운영체계상 왜 별도 재원 조달이 필요한지에 대한 정책적 설명을 요구하고 있음",
                            "답변서 작성 시에는 지방채 인수의 대상 사업, 인수 조건, 절차와 함께 실제 사례를 결합하여 서술하는 것이 이해도를 높이는 데 유리함",
                        ],
                        note: "질의 배경 문단은 실제 답변서의 첫머리에서 배경 설명과 쟁점 정리를 동시에 수행하는 역할을 하므로, 문장 길이를 충분히 유지한 형태가 바람직합니다.",
                    },
                    {
                        title: "2. 활용 가능 문단",
                        desc: "과거 답변서에서 직접 활용 가능한 핵심 문장들은 제도 목적, 예산 편성 배경, 자금지원 방식 등을 간결하면서도 논리적으로 정리하고 있습니다. 초안 반영 시에는 동일한 논리 구조를 유지하되 시점에 따라 최신 수치와 사례만 갱신하는 방식이 효율적입니다.",
                        items: [
                            "공자기금은 지방재정 지원을 목적으로 지자체가 발행하는 지방채를 장기 저리로 인수하여 자금을 지원함",
                            "도로, 지하철 건설, 공공용 시설 설치 및 지역개발사업 등 특정 자본적 지출 사업의 원활한 추진을 위해 추경을 통해 인수 예산을 편성함",
                            "지방교부세는 일반재원으로 폭넓게 활용되나, 지방채 인수는 특정 사업의 적기 집행을 위한 별도 금융지원 수단으로 운용된다는 점을 함께 설명할 필요가 있음",
                        ],
                    },
                ],
            },
            {
                ...
                sections: [
                    {
                        title: "3. 제도 운영 현황",
                        desc: "지방채 인수 제도는 분기별 금리 변동, 장기 상환 구조, 행정안전부와의 협의 절차 등 운영상 특징이 명확합니다. 따라서 답변서에는 단순한 제도 소개를 넘어 인수금리 산정 방식과 상환 조건이 지방재정 지원이라는 목적에 어떻게 부합하는지 설명하는 문단이 필요합니다.",
                        items: [
                            "인수금리는 공자기금 예탁금리와 동일하게 분기별로 고정되어 적용되며, 답변 시점의 최신 분기 수치 확인이 필요함",
                            "인수기간은 5년 거치 10년 분할 상환으로 운영되어 자금 부담을 장기에 걸쳐 분산할 수 있음",
                            "행안부의 지방채 발행계획 수립 및 승인 절차와 연계되어 집행되므로, 관계부처 협의 절차를 답변에 병기하는 것이 적절함",
                        ],
                    },
                    {
                        title: "4. 초안 반영 제안",
                        desc: "현 답변서 초안에 반영할 때는 표 형식의 제도 개요, 사례 중심의 추경 편성 사유, 그리고 재정건전성 관련 보충 문장을 결합하는 방식이 가장 효율적입니다. 금리나 예산 수치 등 변동 가능성이 있는 값은 반드시 최신 자료를 기준으로 갱신해야 합니다.",
                        items: [
                            "지원대상/지원사업/지원조건/집행절차 표는 과거답변의 틀을 그대로 활용 가능",
                            "금리 수치(예: 2026년 2분기 3.435%)는 최신 고시 기준으로 재확인 필요",
                            "채무비율 관리 계획 또는 사후 관리 방안 문장을 추가하면 답변의 완성도를 높일 수 있음",
                        ],
                    },
                ],
            ...
            },
            {
                ...
                sections: [
                    {
                        title: "5. 비교 관점 및 사례",
                        desc: "광주·전남 통합특별시 사례는 이번 질의에서 추경 편성의 직접적 배경으로 제시할 수 있는 대표 사례입니다. 사례를 설명할 때는 출범 일정, 추경 규모, 지자체별 세부 수요를 함께 적시해야 정책적 설득력이 확보됩니다.",
                        items: [
                            "광주·전남 통합특별시(2026.7.1. 출범) 사례가 추경 편성의 직접적 근거로 활용 가능",
                            "지방채 인수 예산 현황(2026년 예산 12,100억 → 2026년 추경안 2,000억)과 함께 증감 배경을 설명하는 것이 적절함",
                            "전남 700억 원, 광주 195억 원 등 세부 수요를 병기하면 답변의 구체성이 높아짐",
                        ],
                    },
                    {
                        title: "6. 답변서 작성 메모",
                        desc: "답변서 최종본에는 지나치게 장황한 표현을 줄이되, 근거가 되는 문장과 수치를 충분히 남겨야 합니다. 따라서 비교 문서와의 차이를 확인하면서도 기준 문서의 설명 구조를 유지하는 방식으로 수정하는 것이 권장됩니다.",
                        items: [
                            "질문 요지 → 제도 설명 → 사례 제시 → 재정건전성 보완 설명의 순서를 유지",
                            "출처번호와 근거문단을 병기하여 검증 단계에서 재사용할 수 있도록 구성",
                            "실제 제출 전에는 최신 수치와 관계부처 협의 현황을 다시 확인",
                        ],
                    },
                ],
            },
        ...
        const pages = [
            {
                ...
                sections: [
                    {
                        title: "1. 질의 요지",
                        desc: "이번 질의의 핵심은 지방교부세가 이미 지급되고 있음에도 왜 추가로 지방채 인수 추경을 편성해야 하는지에 대한 정책적 이유를 설명하는 것입니다. 단순한 예산 필요성 설명을 넘어, 지방재정 지원수단의 성격 차이와 특정 자본사업의 집행 필요성을 함께 기술하는 것이 중요합니다.",
                        items: [
                            "지방교부세 지급에도 지방채 인수로 채무가 증가하는데 왜 추경 편성이 필요한지에 대한 질의",
                            "일반재원과 목적성 재원 간 차이, 지방채 인수의 정책 목적, 재정건전성 영향 설명이 필요",
                            "추경 편성이 단순 예산 확대가 아니라 특정 사업의 적기 집행을 위한 재원 조달이라는 점을 분명히 해야 함",
                        ],
                    },
                    {
                        title: "2. 답변 내용",
                        desc: "초안은 공자기금 지방채 인수의 목적과 제도 구조를 중심으로 답변 논리를 구성하고 있습니다. 비교 문서와의 유사성을 유지하되, 현재 질의에 맞는 사례와 수치를 반영하는 방식으로 정리하였습니다.",
                        items: [
                            "공자기금은 지방재정 지원을 목적으로 지방채를 <mark>장기 저리</mark>로 인수하여 자금을 지원함",
                            "특정 자본적 지출 사업(도로, 지하철, 공공시설, 지역개발)의 원활한 추진을 위해 추경 편성 필요",
                            '<span class="hl-change"><strong>인수조건:</strong> 5년 거치 10년 분할 상환, 2026년 2분기 금리 3.435%</span>',
                            "광주·전남 통합특별시 출범 관련 약 1,000억 원 규모 추경 수요를 사례로 반영",
                        ],
                    },
                ],
            },
            {
                ...
                sections: [
                    {
                        title: "3. 예산 현황",
                        desc: "답변서 초안에서는 최근 연도의 예산과 결산, 본예산과 추경안의 차이를 제시하여 추경 규모의 타당성을 설명하고 있습니다. 특히 전년도 집행 실적과 당해 연도 추경안 간의 관계를 함께 설명하면 설득력이 높아집니다.",
                        items: ["2026년 예산(최종) 12,100억 원 → 결산 10,712억 원", "2026년 본예산 1,000억 원 → 추경안 2,000억 원(증감 +1,000, 100%)", "전년도 집행 실적과 인수잔액을 함께 제시하면 재정 운용의 연속성을 설명하는 데 유용함"],
                    },
                    {
                        title: "4. 사례 및 근거",
                        desc: "광주·전남 통합특별시 사례는 추경 편성의 구체적 수요를 설명하는 데 가장 직접적인 근거입니다. 초안에는 출범 시점과 지자체별 수요를 함께 서술하여 실제 사업 필요를 뒷받침하고 있습니다.",
                        items: [
                            "광주·전남 통합특별시 출범(2026.7.1. 예정)과 관련된 통합전산망 구축, 안내표지판 설치 등의 수요 발생",
                            "전남 700억 원, 광주 195억 원 등 지자체별 세부 수요를 반영하여 총 1,000억 원 규모 추경 수요 설명",
                            "사례 설명 뒤에는 재정건전성 관리 및 집행 절차 보완 문장을 덧붙이는 것이 바람직함",
                        ],
                    },
                ],
            ...
            },
            {
                ...
                sections: [
                    {
                        title: "5. 향후 계획",
                        desc: "국회 심의가 완료된 이후에는 지자체별 세부 사업계획과 집행 절차에 따라 지방채 인수 지원이 순차적으로 이루어집니다. 따라서 답변서에는 제도 설명뿐 아니라 향후 집행 관리 계획을 간단히 함께 서술하는 것이 적절합니다.",
                        items: [
                            "추경안 국회 심의 완료 후 지자체별 세부 사업계획에 따라 순차 집행 예정",
                            "분기별 금리 변동 사항은 예탁금리 고시에 따라 갱신하여 안내",
                            "관계부처 및 지자체와의 협의를 통해 사업별 집행 시기와 규모를 재점검할 예정",
                        ],
                    },
                    {
                        title: "6. 수정 필요 메모",
                        desc: "현 초안은 전반적인 논리 구조가 적정하나, 비교 기준 문서와 대조했을 때 일부 수치 확인과 표현 다듬기가 필요한 부분이 존재합니다. 이 메모는 차이점 분석과 함께 보며 수정 우선순위를 정하는 데 활용할 수 있습니다.",
                        items: [
                            "금리 3.435%는 최신 분기 고시 기준으로 다시 확인 필요",
                            "지방교부세와 지방채 인수의 재원 성격 차이를 한 문장 더 명확히 설명하는 것이 바람직함",
                            "사례 문단 뒤에 재정건전성 관리 계획을 연결하면 국회 답변 톤에 더 부합함",
                        ],
                    },
                ],
            },
  ...
  `;
    ...
    function renderReferenceFileIcon(file) {
        ...
        const processingClass = file?.status && file.status !== "done" ? " processing" : "";
        return `<span class="file-dot file-type-${type}${processingClass}" aria-hidden="true"></span>
      <span class="file-collapsed-ext-icon file-type-${type}${processingClass}" aria-hidden="true"><span>${escapeHtml(ext)}</span></span>`;
    }
    ...
    function renderFiles() {
        ...
        ${renderReferenceFileIcon(f)}
        <span class="file-name-simple">${escapeHtml(f.name)}</span>
        ${renderFileStatusBadge(f)}
        <button class="file-remove-simple" type="button" data-idx="${i}" aria-label="${escapeHtml(f.name)} 삭제">×</button>
      </li>`;
            })
            .join("");
        ...
        const collapsedReferenceFileCount = $("#collapsedReferenceFileCount");
        const collapsedReferenceCount = $("#collapsedReferenceCount");
        if (collapsedReferenceFileCount) collapsedReferenceFileCount.textContent = files.length;
        if (collapsedReferenceCount) {
            collapsedReferenceCount.setAttribute("aria-label", `참조소스 파일 ${files.length}건`);
            collapsedReferenceCount.title = `참조소스 파일 ${files.length}건`;
        }
    ...
    function renderRecommendations() {
        ...
        $("#recommendList").innerHTML = filtered
            .map((r) => {
                const isSelected = selectedRecIds.includes(r.id);
                const topBadge = r.rank ? `<span class="rec-top-badge${r.rank === 1 ? " rec-top-badge--first" : ""}">TOP ${r.rank}</span>` : "";
                return `
    ...
    function renderChatTopics() {
        ...
        el.innerHTML = visibleTopics.length
            ? visibleTopics
                  .map(
                      ({ topic: t, index: i }) =>
        ...
        <div class="ct-menu hidden" data-menu-idx="${i}">
          ...
          <button class="ct-menu-item ct-menu-danger" data-action="delete"><svg viewBox="0 0 24 24" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>삭제</button>
                  ...
                  .join("")
            : '<div class="drawer-list-empty">검색 결과가 없습니다.</div>';
    ...
    function renderChatMessages() {
        ...
        <button class="msg-action-btn" type="button" data-action="like" data-idx="${i}" aria-label="좋아요" aria-pressed="false" title="좋아요"><svg viewBox="0 0 24 24" width="14" height="14"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3H14zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"/></svg></button>
        <button class="msg-action-btn" type="button" data-action="dislike" data-idx="${i}" aria-label="싫어요" aria-pressed="false" title="싫어요"><svg viewBox="0 0 24 24" width="14" height="14"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3H10zM17 2h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"/></svg></button>
        <button class="msg-action-btn" type="button" data-action="retry" data-idx="${i}" aria-label="다시 생성" title="다시 생성"><svg viewBox="0 0 24 24" width="14" height="14"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button>
        <button class="msg-action-btn" type="button" data-action="copy" data-idx="${i}" aria-label="복사" title="복사"><svg viewBox="0 0 24 24" width="14" height="14"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>
        <button class="msg-action-btn msg-report-btn" type="button" data-action="report" data-idx="${i}" aria-label="오류신고" title="오류신고"><svg viewBox="0 0 24 24" width="14" height="14"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg></button>
        <button class="msg-action-btn" type="button" data-action="more" data-idx="${i}" aria-label="더보기" title="더보기">···</button>
      </div>`
                        : "";
                const isTyping = m.role === "ai" && m.typing;
                const textHtml = isTyping ? "" : escapeHtml(m.text).replace(/\n/g, "<br>");
                const avatar = m.role === "ai"
                    ? '<div class="msg-avatar"><img src="../assets/icons/ai-search.svg" alt="" aria-hidden="true" /></div>'
                    : "";
                return `<div class="chat-msg ${m.role}${isTyping ? " is-typing" : ""}" data-component="chat-message" data-variant="answer" data-role="${m.role}" data-status="complete">${avatar}<div class="msg-text">${textHtml}</div>${m.time ? `<span class="msg-time">${escapeHtml(m.time)}</span>` : ""}${actions}</div>`;
            })
            .join("");
        el.scrollTop = el.scrollHeight;

        // 오류신고·더보기는 Answer 페이지가 소유하고 나머지는 ChatMessage가 처리합니다.
        $$('[data-action="report"], [data-action="more"]', el).forEach((btn) => {
            btn.addEventListener("click", (e) => {
                e.stopPropagation();
    ...
    function bindAnswerChatMessageActions() {
        const messageList = $("#chatMessages");
        if (!messageList || messageList.dataset.answerActionsReady === "true") return;
        ...
        messageList.dataset.answerActionsReady = "true";
    }
    ...
    function typeWriterEffect(target, fullText, onDone, scrollEl) {
        ...
        function step(now) {
            ...
            target.innerHTML = escapeHtml(fullText.slice(0, len)).replace(/\n/g, "<br>") + (isDone ? "" : '<span class="typing-cursor"></span>');
            if (scrollEl) scrollEl.scrollTop = scrollEl.scrollHeight;
            if (!isDone) {
                requestAnimationFrame(step);
            } else {
                target.innerHTML = escapeHtml(fullText).replace(/\n/g, "<br>");
                onDone && onDone();
            }
        }
        requestAnimationFrame(step);
        ...
        <div class="center-split" id="tabRecommend">
          <div class="center-left">
            ...
            <div class="filter-bar" role="toolbar" aria-label="관련자료 필터">
              <button class="filter-btn active" type="button" aria-pressed="true" data-filter="all">전체</button>
              <button class="filter-btn" type="button" aria-pressed="false" data-filter="similar">유사답변서</button>
              <button class="filter-btn" type="button" aria-pressed="false" data-filter="reference">참고자료</button>
            </div>
    ...
    function bindRecommendationFilters() {
        if (document.documentElement.dataset.answerFilterReady !== "true") {
            document.addEventListener("filter-btn:change", (event) => {
                if (!event.target.closest("#recommendViewerPanel")) return;
                currentFilter = event.detail.filter || "all";
                renderRecommendations();
                const filtered = currentFilter === "all" ? recommendations : recommendations.filter((recommendation) => recommendation.category === currentFilter);
                if (filtered.length) renderPreview(filtered[0]);
                updateFilterDesc(currentFilter);
                bindApplyToChat();
                bindSelectAll();
            });
            document.documentElement.dataset.answerFilterReady = "true";
        }
        updateFilterDesc(currentFilter);
        bindSelectAll();
    }
    ...
    function resetAnswerWorkspace() {
        ...
        $$(".top-tab").forEach((tab, index) => tab.classList.toggle("active", index === 0));
    ...
    function bindEvents() {
        ...
        $$(".nav-link", sidebar).forEach((l) =>
            l.addEventListener("click", () => {
                if (l.dataset.page === "home" || l.dataset.soonTarget) return; // AI-ONE 홈·준비중 이동은 접지 않음
                localStorage.setItem("sidebar-collapsed", "true");
        ...
        // New Chat button: 새 대화 생성과 동시에 현재 워크스페이스 업무 상태를 초기화
        ...
        if (resetBtn) {
            resetBtn.addEventListener("click", () => {
                resetAnswerWorkspace();
                chatConversations[activeChatTopic] = [{ role: "ai", text: AI_CHAT_INTRO, time: new Date().getHours() + ":" + String(new Date().getMinutes()).padStart(2, "0") }];
                renderChatMessages();
                showToast("초기화되었습니다.");
            });
        }
    ...
    function showToast(msg) {
        window.AIOneToast.show(msg, { target: "#toast", duration: 2000 });
    }
